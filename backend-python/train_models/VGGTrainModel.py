...
from main_app import vgg_cnn_lstm_model, data_generator
...
class VGGTrainModel(BaseClassTrainModel):
    ...
    def train_model(self, dataset, cnn_model, denseLayersForCNN, lstm_model, denseLayersForLSTM, dropOutRate,
        ...
        train_images, train_descriptions, train_features, vocab_size, max_length, tokenizer = \
            TrainDatasetLoader.dataset_loader(fileWithImageNameOnly, fileWithCaptionAndImageName, uniqueValueForFile,
                                              descriptionsDictionary, "VGG16")

        model = vgg_cnn_lstm_model.define_model(vocab_size, max_length, denseLayersForCNN, denseLayersForLSTM,
        ...
        for i in range(epochs):
            ...
            model_filename = f"{nameOfModel}___VGG16___{i}___{uniqueValueForFile}.keras"
            model_path = os.path.join(sub_directory, model_filename)
            model.save(model_path)

            model_architecture_image_path = os.path.join(sub_directory, f"{nameOfModel}___VGG16___{i}.png")
        ...
        save_loss_chart(epochs_info, f"{nameOfModel}___VGG16___'Epochs'___{uniqueValueForFile}", nameOfModel)
        ...
        response_data = {
            'Model Name': 'Your Model will be saved under the name - ' f"{nameOfModel}___VGG16___'Epochs'___{uniqueValueForFile}",
